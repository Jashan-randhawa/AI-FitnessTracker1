...
export default function DateDropdown({ logs, colorClass = "text-emerald-400", label = "Daily Log", selectedDate, onSelectDate }: Props) {
  ...
  return (
    <div ref={rootRef} className="relative inline-flex items-center gap-3 pl-4 pr-2 py-2 rounded-2xl bg-slate-900 dark:bg-slate-900 border border-slate-800">
      {/* Icon + label */}
      <div className="flex items-center gap-2">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
          strokeLinecap="round" strokeLinejoin="round" className={colorClass}>
          <rect x="3" y="5" width="18" height="16" rx="3" />
          <path d="M8 3v3M16 3v3" />
          <circle cx="12" cy="13" r="1" fill="currentColor" />
        </svg>
        <span className="text-white font-semibold text-base whitespace-nowrap">{label}</span>
      </div>
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-600 text-white text-base hover:border-slate-400 transition-colors cursor-pointer"
      >
